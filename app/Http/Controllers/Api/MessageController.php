...
class MessageController extends Controller
{
    ...
    public function readConversation(Request $request){
        $messages = array(
            'match_id.required'    => 'Match Id field is required.',
            'type'  => 'required|in:private_chat,normal_chat',
        );
    ...
    public function matchDetails(Request $request)
    {
      ...
      ///  $allLikes        = UsersLikes::where('like_to', $user->id)->where('match_status', 'match')->where('match_as','!=','private_request')->orderBy('match_id','DESC')->get();
        $allLikes        = UsersLikes::where('like_to', $user->id)->where('match_status', 'match')->whereNull('match_as')->orderBy('match_id','DESC')->get();
      //  print_r($allLikes);exit;
...
//print_r($matchId);exit;
    ...
    public function rejectPrivateChatRequest(Request $request)
    {
        ...
        $check = UserPrivateChat::with('getRequestFromUser')
            ...
            ->where(['request_from'=>$request->user_id,'request_to'=>$user->id,'request_status'=>'requested'])
            ->first();
