...
<div id="forgot-password" class="row">
  <div class="col s12 m6 l4 z-depth-4 offset-m4 card-panel border-radius-6 forgot-card bg-opacity-8">
    ...
    @endif
    <form class="login-form" method="POST" action="{{ url('') }}/admin_change_new_password">
      @csrf
       @if (session('id'))
      <input type="hidden" name="key" value="{{session('id')}}">
      @else
        <input type="hidden" name="key" value="{{ old('key') }}">
      @endif
      <div class="row">
        <div class="input-field col s12">
          <h5 class="ml-4">New Password</h5>
          <p class="ml-4"></p>
        </div>
      </div>
      <div class="row">
        <div class="input-field col s12">
          ...
          <input id="email" type="text" class="form-control @error('email') is-invalid @enderror" name="new_password"
            value="{{ old('email') }}" autocomplete="email" autofocus>
          <label for="email" class="center-align">Password</label>
          @error('new_password')
